...
export function updatePlan(id, { name, amount_cents, tokens }) {
  getDb().prepare(
    "UPDATE plans SET name = ?, amount_cents = ?, tokens = ?, updated_at = datetime('now') WHERE id = ?"
  ).run(name, amount_cents, tokens, id);
}
...
export function searchUsers(query, limit = 25) {
  const q = `%${query || ''}%`;
  return getDb().prepare(
    'SELECT id, email, credits_balance, created_at, google_id, facebook_id FROM users WHERE email LIKE ? ORDER BY created_at DESC LIMIT ?'
  ).all(q, limit);
}
