...
function getDb() {
  ...
  // Último status bruto que a Muapi respondeu enquanto o job ainda estava
  // "pending" — atualizado a cada poll (ver updateLastStatusRaw). Serve de
  // rede de segurança: se um job travar e nunca receber um erro explícito
  // (ex: o cliente desiste de esperar depois de ~30min sem nunca avisar o
  // servidor do motivo), ainda temos a última coisa que a Muapi disse,
  // pra usar como motivo do erro em vez de deixar em branco.
  if (!genCols.includes('last_status_raw')) {
    _db.exec("ALTER TABLE generations ADD COLUMN last_status_raw TEXT");
  }
  ...
  // Conteúdo editável do e-mail de boas-vindas (enviado automaticamente a
  // todo novo cadastro — ver lib/welcomeEmail.js). "Own check" — mesma
  // lógica das outras chaves acima — pra instalações que já existiam
  // ganharem esses valores padrão também.
  if (getSetting('welcome_email_badge', null) === null) {
    setSetting('welcome_email_badge', 'Cadastro confirmado');
  }
  if (getSetting('welcome_email_headline', null) === null) {
    setSetting('welcome_email_headline', 'Sua conta na VisuIA está pronta!');
  }
  if (getSetting('welcome_email_body', null) === null) {
    setSetting(
      'welcome_email_body',
  ...
  if (getSetting('welcome_email_button_text', null) === null) {
    setSetting('welcome_email_button_text', 'Começar a criar');
  }
...
//
// If the caller doesn't have a specific error message (e.g. the admin
// "Corrigir" button resolving a job that just got stuck in "pending"
// forever, with the client having given up on polling without ever
// telling the server why), we fall back to the last raw status Muapi
// gave us for this job (see updateLastStatusRaw) instead of leaving
// error_message blank.
export function settleGenerationFailure(id, errorMessage) {
  return withTransaction(() => {
    ...
    let finalMessage = errorMessage ? String(errorMessage) : null;
    if (!finalMessage && job.last_status_raw) {
      finalMessage = `Sem erro explícito — último status conhecido da Muapi antes de travar: ${job.last_status_raw}`;
    }

    getDb().prepare('UPDATE generations SET status = ?, error_message = ? WHERE id = ?')
      .run('failed', finalMessage ? finalMessage.slice(0, 500) : null, id);
    return getGenerationById(id);
  });
}

// Called from /api/generate/poll on every "still processing" check —
// keeps a running snapshot of the last thing Muapi said about this job
// while it's still pending, so if it eventually gets stuck and resolved
// without an explicit error, settleGenerationFailure has something real
// to fall back on instead of nothing.
export function updateLastStatusRaw(id, raw) {
  if (!raw) return;
  const text = typeof raw === 'string' ? raw : JSON.stringify(raw);
  getDb().prepare('UPDATE generations SET last_status_raw = ? WHERE id = ?')
    .run(text.slice(0, 1000), id);
}
...
export function getRecentFailures({ days = 7, date } = {}) {
  ...
  if (date) {
    return db.prepare(`
      SELECT g.id, g.model, g.kind, g.error_message, g.last_status_raw, g.created_at, u.email AS user_email
      FROM generations g
