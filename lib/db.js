...
export function createUserWithGoogle(email, googleId) {
  ...
  ).run(email, '', Number(process.env.SIGNUP_FREE_CREDITS || 0), googleId);
  return getUserById(Number(info.lastInsertRowid));
}
...
export function createSubscription(userId, stripeSubscriptionId, stripeCustomerId, plan, status) {
  getDb().prepare(
    'INSERT INTO subscriptions (user_id, stripe_subscription_id, stripe_customer_id, plan, status) VALUES (?, ?, ?, ?, ?)'
  ).run(userId, stripeSubscriptionId, stripeCustomerId, plan, status);
}
...
// The one the user sees in /conta — their most recent active (or
// canceling-but-not-yet-ended) subscription, if any.
export function getActiveSubscriptionForUser(userId) {
  return getDb().prepare(
    "SELECT * FROM subscriptions WHERE user_id = ? AND status IN ('active', 'canceling') ORDER BY created_at DESC LIMIT 1"
  ).get(userId);
}
